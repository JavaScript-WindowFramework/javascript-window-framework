...
namespace JWF {
	...
	export class Window {
		...
		private addFrame(titleFlag: boolean): void {
			...
			let titleText = WindowManager.createElement("div", { "dataset": { jwfType: "text" } })
			title.appendChild(titleText)
			//アイコンの作成
			let icons = ["min", "max", "close"]
			for (let index in icons) {
				let icon = WindowManager.createElement("div", { style: { "width": this.JData.titleSize + "px", "height": this.JData.titleSize + "px" }, "dataset": { jwfType: "icon", jwfKind: icons[index] } })
		...
		*/
		...
		setMaximize(flag: boolean): void {
			...
			if (flag) {
				let icon = this.hNode.querySelector("*>[data-jwf-type=title]>[data-jwf-type=icon][data-jwf-kind=max]") as HTMLElement
				if (icon)
					icon.dataset.jwfKind = "normal"
			} else {
				let icon = this.hNode.querySelector("*>[data-jwf-type=title]>[data-jwf-type=icon][data-jwf-kind=normal]") as HTMLElement
				if (icon)
		...
		setMinimize(flag: boolean): void {
			...
			if (flag) {
				let icon = this.hNode.querySelector("*>[data-jwf-type=title]>[data-jwf-type=icon][data-jwf-kind=min]") as HTMLElement
				icon.dataset.jwfKind = "restore"
			} else {
				let icon = this.hNode.querySelector("*>[data-jwf-type=title]>[data-jwf-type=icon][data-jwf-kind=restore]") as HTMLElement
